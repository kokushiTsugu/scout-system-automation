...
const JOB_CATALOG_MAX_JOBS = 120;
const JOB_CATALOG_MAX_CHARS = 120000;
...
function normalizeProfileText(profile) {
  const normalized = String(profile == null ? '' : profile)
    .replace(/\r\n?/g, '\n')
    .replace(/[\u3000\t]+/g, ' ')
    .trim();
  return clampText(normalized, PROFILE_MAX_CHARS);
}
...
function _runBatchScoutMatching() {
  ...
  for (let i = 1; i < rows.length && handled < MAX_BATCH; i++) {
    ...
    try {
      ...
      const profileForPrompt = normalizeProfileText(profile);
      const prompt  = buildPrompt('{姓}', profileForPrompt, jobCatalogPayload);
      _log('prompt stats', { RID, row: rowIdx, promptChars: prompt.length, profileChars: profileForPrompt.length });
      let aiText;
      let data;
