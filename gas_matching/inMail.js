...
# 厳守すべきルール
1. **呼称統一**: 候補者の呼称は必ず「${lastName} 様」とする。
...
3. **JSON項目作成**: 以下の指示に従い、各項目を生成する。
    ...
    - **subject**: 「【限定スカウト】${lastName}様のご経歴に親和性の高い、[No.1求人のcompany_desc]のポジションのご案内」という形式で作成する。
    - **intro_sentence**: 候補者の経歴から最も特筆すべき実績（在籍企業名＋具体的な役割）を引用し、「…してこられたご経歴に強く惹かれ、限定スカウトをお送りいたしました。」という自然で簡潔な一文を作成する。
    - **closing_sentence**: 「上記以外にも「広告×データ領域でのBizDev」「外資SaaSの日本ローンチ」など、${lastName} 様のご志向に沿うポストを複数ご用意しています。」のように、紹介案件以外にも魅力的な選択肢があることを示唆する一文を作成する。
...
# お手本（このフォーマットとトーンを参考にしてください）
{
  ...
  "subject": "【限定スカウト】Akutsu様のご経歴に親和性の高い、グローバル Commerce Media プラットフォームのポジションのご案内",
